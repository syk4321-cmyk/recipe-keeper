...
export default function RecipeKeeper() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("recipes", false);
        if (r && r.value) {
      ...
      try {
        const f = await window.storage.get("folders", false);
        if (f && f.value) setFolders(JSON.parse(f.value));
      } catch (e) {}
      try {
        const l = await window.storage.get("cardLayout", false);
        if (l && l.value) setCardLayout(l.value === "grid" ? "grid" : "list");
      } catch (e) {}
      try {
        const fs = await window.storage.get("featureSuggestions", true);
        if (fs && fs.value) setSuggestions(JSON.parse(fs.value));
      } catch (e) {}
      try {
        const vi = await window.storage.get("votedSuggestionIds", false);
        if (vi && vi.value) setVotedIds(JSON.parse(vi.value));
      } catch (e) {}
      try {
        const s = await window.storage.get("shoppingList", false);
        if (s && s.value) {
  ...
  useEffect(() => { if (ready) window.storage.set("recipes", JSON.stringify(recipes), false).catch(() => {}); }, [recipes, ready]);
  useEffect(() => { if (ready) window.storage.set("folders", JSON.stringify(folders), false).catch(() => {}); }, [folders, ready]);
  useEffect(() => { if (ready) window.storage.set("cardLayout", cardLayout, false).catch(() => {}); }, [cardLayout, ready]);
  useEffect(() => { if (ready) window.storage.set("featureSuggestions", JSON.stringify(suggestions), true).catch(() => {}); }, [suggestions, ready]);
  useEffect(() => { if (ready) window.storage.set("votedSuggestionIds", JSON.stringify(votedIds), false).catch(() => {}); }, [votedIds, ready]);
  ...
  useEffect(() => { if (ready) window.storage.set("shoppingList", JSON.stringify(shoppingList), false).catch(() => {}); }, [shoppingList, ready]);
